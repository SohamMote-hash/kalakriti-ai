...
export function runMarketAnalysis(target: Product, allProducts: Product[]): MarketAnalysisResult {
  ...
  const averagePrice = prices.length ? Math.round(prices.reduce((a, b) => a + b, 0) / prices.length) : target.price;
  ...
  const positionVsAverage = averagePrice > 0 ? ((target.price - averagePrice) / averagePrice) * 100 : 0;
