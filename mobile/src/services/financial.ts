...
export function calculateFinancials(inputs: FinancialInputs): FinancialResults {
  const { materialCost, labourCost, packagingCost, transportCost, otherCost, productionQuantity, sellingPrice } =
    inputs;

  const costPerUnit = materialCost + labourCost + packagingCost + transportCost + otherCost;
  ...
  const breakEvenUnits = profitPerUnit > 0 ? Math.ceil(totalMonthlyCost / sellingPrice) : Infinity;
